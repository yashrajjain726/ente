...
class NotesProvider(private val context: Context) {
    ...
    private suspend fun <T> withAccess(block: suspend () -> T): T =
        withContext(Dispatchers.IO) { diskGate.withLock { block() } }
                ...
                for (registered in registrations.values) {
                    val other =
                        AndroidNotesSource(
                            ...
                            Uri.parse(registered.tree),
                            token,
                        )
                    val overlaps =
                        source.sameRoot(other) ||
                    ...
                    if (overlaps) {
                        throw NotesException.InvalidInput(
                            "This folder overlaps a folder already added to Your Notes."
                        )
                    }
                }
    ...
    internal suspend fun preview(reference: NoteSourceReference): Uri = withAccess {
        ...
        previews
            .listFiles()
            ?.filter { it.lastModified() < System.currentTimeMillis() - 86_400_000L }
            ?.forEach { it.deleteRecursively() }
        val folder = File(previews, UUID.randomUUID().toString()).apply { mkdirs() }
        val file =
            File(folder, reference.documentId.substringAfterLast('/')).apply { writeBytes(bytes) }
    ...
    private fun loadRegistry(): List<Registration> {
        ...
        root.listFiles()?.forEach { child ->
            if (
                child.name !in registeredIds &&
                    runCatching { UUID.fromString(child.name).toString() == child.name }
                        .getOrDefault(false)
            ) {
