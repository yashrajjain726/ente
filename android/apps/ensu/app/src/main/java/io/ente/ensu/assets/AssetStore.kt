...
class AssetStore(context: Context) {
    ...
    private fun legacyAssets(modelUrl: String?, mmprojUrl: String?) =
        LegacyAssets(
    ...
    fun llmModelPath(asset: Asset): File? = core.llmModelPath(asset)?.let(::File)

    fun llmMmprojPath(asset: Asset): File? = core.llmMmprojPath(asset)?.let(::File)
    ...
    suspend fun estimateDownloadSize(asset: Asset): Long? =
        withContext(Dispatchers.IO) { core.estimatedDownloadSize(asset) }
