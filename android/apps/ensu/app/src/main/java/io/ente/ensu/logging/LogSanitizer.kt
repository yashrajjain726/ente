...
private val keyValueSecretRegex =
    Regex(
...
private val querySecretRegex =
    Regex("(?i)(token|key|sig|signature|auth|session|passkey|otp)=([^&\\s]+)")
...
internal fun sanitizeLog(input: String?): String? {
    ...
    out =
        keyValueSecretRegex.replace(out) { match ->
            val key = match.groups[1]?.value ?: "secret"
            "$key=<redacted>"
        }
    out =
        querySecretRegex.replace(out) { match ->
