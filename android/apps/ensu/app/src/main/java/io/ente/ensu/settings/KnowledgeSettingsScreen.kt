        ...
        if (pack.isMutating) {
            ...
            Row(
                ...
                LinearProgressIndicator(
                    progress = {
                        (pack.mutationProgress?.percentage ?: 0.0).coerceIn(0.0, 100.0).toFloat() /
                            100f
                    },
                    modifier = Modifier.weight(1f),
            ...
            packDownloadsAllowed && pack.status == KnowledgeReconciliationStatus.UPDATE_AVAILABLE
        ) {
            Spacer(Modifier.height(EnsuSpacing.md.dp))
            CompactButton(label = "Update", onClick = onDownloadOrUpdate)
        }

        pack.errorMessage?.let { Text(it, style = EnsuTypography.small, color = EnsuColor.error) }
    }
}
    ...
    AttributionDialog(title = "Attribution", onDismiss = onDismiss) {
        KnowledgeCard(padding = EnsuSpacing.md.dp, spacing = EnsuSpacing.sm.dp) {
            ...
            Row(
                ...
                AttributionLink("Source ↗") { uriHandler.openUri(attribution.publicPackUrl) }
                AttributionLink("License ↗") { uriHandler.openUri(attribution.licenseUrl) }
            }
            HorizontalDivider(color = EnsuColor.border())
            Text(
                text =
