...
internal fun MessageList(
    ...
    val bottomItemIndex by
        remember(messages.size, isGenerating) {
            derivedStateOf {
                messages.size + (if (isGenerating) 1 else 0) + (if (messages.isNotEmpty()) 1 else 0)
            }
        }
    ...
    LaunchedEffect(lastMessage?.id) {
        if (lastMessage == null) {
            ...
            lastMessage.author == MessageAuthor.User && lastMessage.id != lastUserMessageId
        ) {
    ...
    LazyColumn(
        ...
        if (messages.isNotEmpty()) {
            item(key = "ai-safety-footer") { AiSafetyFooter() }
        }
...
private fun AiSafetyFooter() {
    Text(
        ...
        color = EnsuColor.textMuted(),
        textAlign = TextAlign.Center,
        modifier = Modifier.fillMaxWidth().padding(top = EnsuSpacing.sm.dp),
    )
}
...
private fun DownloadOnboarding(
    ...
    val sizeText =
        modelDownloadSizeBytes?.let { "Approx. ${formatBytes(it)}" }
            ...
            if (isDownloading) {
                ...
                val statusText =
                    when {
                        ...
                        modelDownloadSizeBytes != null &&
                            ...
                            downloadPercent >= 0 -> {
                            val downloadedBytes =
                                (modelDownloadSizeBytes * (downloadPercent / 100f)).roundToLong()
                ...
                if (!downloadStatus.isNullOrBlank()) {
                    Text(
                        ...
                        style = EnsuTypography.body,
                        color =
                            if (downloadPhase == DownloadPhase.Failed) {
                                EnsuColor.error
                            } else {
                                EnsuColor.textMuted()
                            },
...
private fun UserMessageBubble(
    ...
    val bubbleFill =
        if (isSystemInDarkTheme()) EnsuColor.fillFaint() else EnsuColor.border().copy(alpha = 0.2f)

    Column(
        modifier = Modifier.fillMaxWidth().padding(start = EnsuSpacing.messageBubbleInset.dp),
                ...
                message.attachments.forEach { attachment ->
                    if (attachment.type == AttachmentType.Image && attachment.localPath != null) {
                        ...
                        io.ente.ensu.components.AttachmentChip(
                            ...
                            isUploading = attachment.isUploading,
                            onClick =
                                if (attachment.localPath == null) null
                                else {
        ...
        Box {
            Column(
                modifier =
                    Modifier.background(bubbleFill, bubbleShape)
                        .pointerInput(Unit) {
                            detectTapGestures(
                                onLongPress = { offset ->
                    ...
                    if (message.branchCount > 1) {
                        BranchSwitcher(
                            ...
                            onNext = {
                                ...
                                onBranchChange(
                                    message.id,
                                    (current + 1).coerceAtMost(message.branchCount),
                                )
                            },
                        )
                    }
...
private fun KnowledgeSourcesDialog(
    ...
    val notesState =
        if (citations.any { it is GroundedSource.LocalNote }) notes?.state?.collectAsState()?.value
        else null
        ...
        citations.forEachIndexed { index, source ->
            ...
            KnowledgeCard(padding = EnsuSpacing.md.dp, spacing = EnsuSpacing.sm.dp) {
                ...
                note
                    ...
                    ?.let { Text(it, style = EnsuTypography.small, color = EnsuColor.textMuted()) }
                ...
                FlowRow(horizontalArrangement = Arrangement.spacedBy(EnsuSpacing.lg.dp)) {
                    if (note != null) {
                        ...
                        AttributionLink("${citation.licenseLabel} ↗") {
...
private fun MessageActionsMenu(
    ...
    Popup(
        ...
        Surface(
            ...
            Column(
                modifier = Modifier.width(IntrinsicSize.Min).padding(vertical = EnsuSpacing.xs.dp)
            ) {
