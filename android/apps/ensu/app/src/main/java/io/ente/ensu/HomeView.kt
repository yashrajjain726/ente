...
fun HomeView(
    ...
    CompositionLocalProvider(LocalNotesStore provides store.notesStore) {
        ...
        val currentSession by
            remember(appState.chat.sessions, appState.chat.currentSessionId) {
                derivedStateOf {
        ...
        if (showLogShareDialog) {
            ModalBottomSheet(
                ...
                Column(
                    ...
                    Text(
                        text = "Export or share logs for support.",
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(EnsuSpacing.sm.dp)) {
                        TextButton(
                            ...
                            colors =
                                ButtonDefaults.textButtonColors(
                                    contentColor = EnsuColor.textPrimary()
                                ),
                        ) {
                            Text(text = "Share")
                        }
                        ...
                        TextButton(
                            ...
                            colors =
                                ButtonDefaults.textButtonColors(
                                    contentColor = EnsuColor.textMuted()
                                ),
                        ) {
                            Text(text = "Cancel")
                        }
...
private fun normalizedJpegAttachmentName(name: String?): String {
    val raw =
        name?.replace("\u0000", "")?.replace("\\", "/")?.substringAfterLast("/")?.trim()?.takeIf {
            it.isNotBlank()
        } ?: "image"
