...
internal class AttachmentStoreActions(
    ...
    fun addAttachment(attachment: Attachment) {
        ...
        state.update { appState ->
            ...
            val imageLimitReached =
                attachment.type == AttachmentType.Image &&
                    chat.attachments.count { it.type == AttachmentType.Image } >=
                        MaxImageAttachmentsPerMessage
            if (chat.isGenerating || chat.isDownloading || imageLimitReached) {
                appState.copy(chat = chat.copy(isProcessingAttachments = false))
            } else {
                accepted = true
                appState.copy(
                    chat =
    ...
    private fun deleteIfUnstored(attachment: Attachment) {
        val stored =
            messageStore.values.any { messages ->
