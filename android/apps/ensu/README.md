...
3. Open `android/` in Android Studio and run Ensu's `app` module. If the Android NDK is missing, Android Studio will prompt to install it.
...
```sh
./gradlew :ensu:app:assembleRelease
```
...
```sh
./gradlew :ensu:app:bundleRelease
```
