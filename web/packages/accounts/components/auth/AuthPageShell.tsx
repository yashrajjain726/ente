...
export interface AuthPageShellProps extends React.PropsWithChildren {
    brand: AuthBrandCopy;
    ...
    return (
        <PageRoot>
            ...
            <PageCard>
                ...
                <MobileBrandSlot $height={presentationConfig.mobileBrandHeight}>
                    <BrandPanel
                        brand={brand}
                        presentationConfig={presentationConfig}
                        size="mobile"
...
function BrandPanel({
    ...
    return (
        <BrandPanelRoot
            size={size}
            $wideGamutPrimary={presentationConfig.wideGamutPrimary}
        >
            <Wordmark size={size}>
                <EnteLogo />
            </Wordmark>
            <PanelCopy size={size}>
                <PanelHeadline size={size}>{brand.headline}</PanelHeadline>
                {size !== "mobile" && (
                    <PanelSubtitle size={size}>{brand.subtitle}</PanelSubtitle>
                )}
                {size === "desktop" && (
                    <BulletList>
            ...
            <IllustrationClip
                size={size}
                $heights={presentationConfig.illustrationHeights}
            >
...
const BrandPanelRoot = styled("div", {
    shouldForwardProp: (prop) =>
        prop !== "size" && prop !== "$wideGamutPrimary",
})<{ size: BrandPanelProps["size"]; $wideGamutPrimary?: string }>(
    ({ size, $wideGamutPrimary }) => ({
        width: "100%",
        ...
        ...($wideGamutPrimary && {
            "@supports (color: color(display-p3 1 1 1))": {
                backgroundColor: $wideGamutPrimary,
            },
        }),
    }),
);
...
const IllustrationClip = styled("div", {
    shouldForwardProp: (prop) => prop !== "size" && prop !== "$heights",
})<{
    size: BrandPanelProps["size"];
    $heights: AuthPresentationConfig["illustrationHeights"];
}>(({ size, $heights }) => ({
