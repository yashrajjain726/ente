...
fn generate_frb(target: FrbTarget) -> Result<(), DynError> {
    ...
    let shared_dir = repo_root.join("mobile/packages/frb/lib");
    write_generated_gitignore(&shared_dir)?;
    let mut shared_types = None;
    for app in apps {
        let package_dir = repo_root.join("mobile/apps").join(app);
        generate_frb_package(&package_dir)?;
        let types_path =
            package_dir.join("lib/src/rust/third_party/ente_frb_lib/contacts/types.dart");
        let types =
            fs::read_to_string(&types_path)?.replace("import '../../../frb_generated.dart';\n", "");
        if shared_types
            .as_ref()
            .is_some_and(|previous| previous != &types)
        {
            return Err("Photos and Locker generated different shared Contacts types".into());
        }
        fs::write(shared_dir.join("contacts.dart"), &types)?;
        fs::write(
            types_path,
            "// This file is automatically generated, so please do not edit it.\n\n\
             export 'package:ente_frb/contacts.dart';\n",
        )?;
        shared_types = Some(types);
    }
