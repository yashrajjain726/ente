...
mod tests {
    ...
    #[tokio::test]
    async fn api_requests_send_extra_auth_headers() {
        ...
        let client = HttpClient::new_with_config(HttpConfig {
            ...
            extra_auth_headers: vec![(
                "X-Space-Session-Token".to_string(),
                "space-session-token".to_string(),
            )],
            ..HttpConfig::default()
        })
        .unwrap();
        ...
        assert_eq!(response.id, "extra-auth");
    }
    ...
    async fn object_store_client_only_sends_explicit_headers() {
        ...
        let get_mock = server
            ...
            .match_header("x-space-session-token", Matcher::Missing)
