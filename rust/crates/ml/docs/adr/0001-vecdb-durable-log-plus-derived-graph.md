...
- Keys are strings (non-empty UTF-8, ≤ 256 bytes), not integers: file/face/cluster IDs are used directly, which retires the four AUTOINCREMENT key-mapping tables the app kept in sqlite solely for usearch's u64 keys. Internally keys are interned to u32 slots; the graph, arena, and log replay speak slots.
