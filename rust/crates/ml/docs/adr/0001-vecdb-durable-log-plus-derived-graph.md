...
- vecdb is layered as an IO-free core (graph, arena, SIMD kernels — wasm32-compatible) beneath a native-only storage layer (fs, fsync, locking). The writer lock is std file locking (`File::try_lock`) on the `<path>.lock` sidecar — no `libc`, no `windows-sys` — and is held continuously across compaction, so there is never an unlock window; locking the sidecar rather than the log keeps Windows whole-file lock semantics away from readers and log renames. vecdb's only dependency is `wide` (already in the tree); locking is std file locking and CRC32 is hand-rolled in-module, so vecdb adds zero new Cargo.lock entries.
