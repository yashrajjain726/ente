...
const apps = {
    ...
    "photos-desktop": {
        changelog: "docs/docs/photos/changelog.md",
        label: "desktop",
    },
...
if (!config || !version) {
    throw new Error(
        `Usage: RELEASE_BODY=... node .github/scripts/sync-help-changelog.mjs ${Object.keys(apps).join("|")} <version>`,
    );
}
...
if (at < 0)
    throw new Error(`No "## v" entry to insert before in ${config.changelog}`);

fs.writeFileSync(
    file,
    `${content.slice(0, at)}${section}\n${content.slice(at)}`,
);
