...
final class IOSNotesSource: NotesSource, @unchecked Sendable {
    ...
    init(
        bookmark: Data, cancellation: NotesCancellation,
        coordinator: NSFileCoordinator = NSFileCoordinator()
    ) throws {
        var stale = false
        let url: URL
        do { url = try URL(resolvingBookmarkData: bookmark, bookmarkDataIsStale: &stale) } catch {
    ...
    private func coordinate<T>(_ url: URL, _ body: (URL) throws -> T) throws -> T {
        ...
        guard let result else {
            throw NotesError.SourceRead(detail: "Could not read the Notes folder.")
        }
        return try result.get()
    }
    ...
    private func inventory() throws -> [NotesDocument] {
        ...
        while let (directory, prefix) = pending.popLast() {
            ...
            guard seen.insert(identity).inserted else {
                throw NotesError.SourceRead(detail: "Ambiguous directory structure.")
            }
            let children = try coordinate(directory) {
                try FileManager.default.contentsOfDirectory(
                    at: $0, includingPropertiesForKeys: Array(keys))
            }
            var names = Set<String>()
            for child in children {
                try cancellation.check()
                count += 1
                guard count <= Self.entryLimit else {
                    throw NotesError.InvalidInput(
                        detail: "The Notes folder contains too many entries.")
                }
                let name = child.lastPathComponent
                guard names.insert(name).inserted else {
                    throw NotesError.SourceRead(detail: "Duplicate document names.")
                }
                ...
                do { try validateNotesDocumentId(documentId: id) } catch NotesError.InvalidInput {
                    continue
                }
                if values.isDirectory == true {
                    ...
                    let size: Int
                    if let advertised = values.fileSize {
                        size = advertised
                    } else {
                        do { size = try readBounded(documentId: id).bytes.count } catch ReadError
                            .tooLarge
                        { continue }
                    }
                    guard size <= Self.byteLimit else { continue }
                    total += UInt64(size)
                    guard documents.count < Int(Self.limits.maxCollectionDocuments),
                        total <= Self.limits.maxCollectionSourceBytes
                    else {
                        throw NotesError.InvalidInput(
                            detail: "The Notes folder is too large to index.")
                    }
                    documents.append(
                        NotesDocument(
                            documentId: id, size: UInt64(size),
    ...
    func readDocument(documentId: String) throws -> NotesRead {
        do { return try readBounded(documentId: documentId) } catch ReadError.tooLarge {
    ...
    private func readBounded(documentId: String) throws -> NotesRead {
        ...
        guard parts.allSatisfy({ !$0.hasPrefix(".") }), Self.supported(parts.last ?? "") else {
            throw NotesError.SourceChanged
        }
        let url = parts.reduce(root) { $0.appendingPathComponent($1) }
        return try coordinate(url) { coordinated in
            guard coordinated.standardizedFileURL.path == url.standardizedFileURL.path else {
                throw NotesError.SourceChanged
            }
            ...
            var after = stat()
            guard fstat(fd, &after) == 0, before.st_ino == after.st_ino,
                before.st_size == after.st_size,
                before.st_mtimespec.tv_sec == after.st_mtimespec.tv_sec,
                before.st_mtimespec.tv_nsec == after.st_mtimespec.tv_nsec,
                bytes.count == after.st_size
            else { throw NotesError.SourceChanged }
            return NotesRead(
                bytes: bytes,
                source: NotesDocument(
                    documentId: documentId, size: UInt64(bytes.count),
                    modifiedAtMs: Int64(after.st_mtimespec.tv_sec) * 1000 + Int64(
                        after.st_mtimespec.tv_nsec) / 1_000_000))
        }
    }
