...
`rust/target/ml-assets-cache/`, with SHA-256 verification for every asset. For
local work against a checked-out fixture repo, point the test at the dataset
root:

```sh
ENTE_ML_ASSETS_REPO_DIR=/path/to/test-fixtures/ml/indexing/v1 \
  cargo test -p ente-photos --test ml_indexing --features ml-assets -- --nocapture
```
