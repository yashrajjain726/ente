...
func TestSpaceMessageConversationsAndNotificationsUseSplitActivity(t *testing.T) {
	ctx := context.Background()
	module := newSpaceTestModule(t)
	...
	require.Len(t, conversations, 1)
	require.Equal(t, bobSpace.SpaceID, conversations[0].Friend.SpaceID)
	require.Empty(t, conversations[0].LatestActivity.Type)
	require.False(t, conversations[0].Unread)
	require.Equal(t, int64(0), conversations[0].UnreadCount)

	notifications, nextCursor, err := module.Messages.ListNotifications(ctx, aliceSpace.SpaceID, "", 10)
	require.NoError(t, err)
	require.Empty(t, nextCursor)
	require.Len(t, notifications, 1)
	require.Equal(t, bobSpace.SpaceID, notifications[0].Actor.SpaceID)
	require.Equal(t, "friend_add", notifications[0].Type)
	require.Nil(t, notifications[0].Post)
	...
	require.NoError(t, err)
	require.Equal(t, "message_like", conversations[0].LatestActivity.Type)
	notifications, _, err = module.Messages.ListNotifications(ctx, aliceSpace.SpaceID, "", 10)
	require.NoError(t, err)
	require.Equal(t, "post_like", notifications[0].Type)
	require.NotNil(t, notifications[0].Post)
	require.Equal(t, postID, notifications[0].Post.PostID)
	require.Equal(t, "activity-post-object", notifications[0].Post.ObjectKey.String)
	...
	latestActivityAt, err := module.Messages.GetLatestMessageThreadActivityAt(ctx, aliceID, aliceSpace.SpaceID, bobSpace.SpaceID)
	require.NoError(t, err)
	require.Equal(t, int64(5500), latestActivityAt)
	messagesUnread, err := module.Messages.HasUnreadMessages(ctx, aliceSpace.SpaceID)
	require.NoError(t, err)
	require.True(t, messagesUnread)
	...
	notifications, _, err = module.Messages.ListNotifications(ctx, aliceSpace.SpaceID, "", 10)
	require.NoError(t, err)
	require.Equal(t, "friend_remove", notifications[0].Type)
	require.Equal(t, bobSpace.SpaceID, notifications[0].Actor.SpaceID)
}

func TestSpaceFriendEventsCreateNotificationsForTarget(t *testing.T) {
	ctx := context.Background()
	module := newSpaceTestModule(t)
	...
	require.Empty(t, aliceConversations[0].LatestActivity.Type)
	require.False(t, aliceConversations[0].Unread)
	require.Equal(t, int64(0), aliceConversations[0].UnreadCount)
	...
	require.Empty(t, bobConversations[0].LatestActivity.Type)

	aliceNotifications, _, err := module.Messages.ListNotifications(ctx, aliceSpace.SpaceID, "", 10)
	require.NoError(t, err)
	require.Len(t, aliceNotifications, 1)
	require.Equal(t, "friend_add", aliceNotifications[0].Type)
	require.Equal(t, bobSpace.SpaceID, aliceNotifications[0].Actor.SpaceID)
	require.True(t, aliceNotifications[0].Unread)
	bobNotifications, _, err := module.Messages.ListNotifications(ctx, bobSpace.SpaceID, "", 10)
	require.NoError(t, err)
	require.Empty(t, bobNotifications)
	...
	aliceLatestActivityAt, err := module.Messages.GetLatestNotificationActivityAt(ctx, aliceSpace.SpaceID)
	require.NoError(t, err)
	require.Equal(t, int64(1000), aliceLatestActivityAt)
	require.NoError(t, module.Read.UpsertNotificationsReadMarker(ctx, aliceID, aliceSpace.SpaceID, aliceLatestActivityAt))
	aliceUnread, err = module.Messages.HasUnreadNotifications(ctx, aliceSpace.SpaceID)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Len(t, aliceNotifications, 1)
	require.False(t, aliceNotifications[0].Unread)
	...
	require.Equal(t, "friend_remove", aliceNotifications[0].Type)
	require.True(t, aliceNotifications[0].Unread)
	require.False(t, aliceNotifications[1].Unread)
	aliceLatestActivityAt, err = module.Messages.GetLatestNotificationActivityAt(ctx, aliceSpace.SpaceID)
	require.NoError(t, err)
	require.Equal(t, int64(2000), aliceLatestActivityAt)
}
...
func TestSpaceMessageConversationPreviewUsesLatestActivityWithSeparateUnreadState(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, messagesUnread)
	messageLikesUnread, err := module.Messages.HasUnreadMessageLikes(ctx, aliceSpace.SpaceID)
	require.NoError(t, err)
	require.True(t, messageLikesUnread)

	latestActivityAt, err := module.Messages.GetLatestMessageThreadActivityAt(ctx, aliceID, aliceSpace.SpaceID, bobSpace.SpaceID)
	require.NoError(t, err)
	require.Equal(t, int64(2000), latestActivityAt)
	require.NoError(t, module.Read.UpsertMessageThreadReadMarker(ctx, aliceID, aliceSpace.SpaceID, bobSpace.SpaceID, latestActivityAt))
	...
	require.NoError(t, module.Read.UpsertMessageLikesReadMarker(ctx, aliceID, aliceSpace.SpaceID, 3000))
	conversations, _, err = module.Messages.ListConversations(ctx, aliceID, aliceSpace.SpaceID, "", 10)
	require.NoError(t, err)
	require.False(t, conversations[0].Unread)
	require.Equal(t, int64(0), conversations[0].UnreadCount)
	messageLikesUnread, err = module.Messages.HasUnreadMessageLikes(ctx, aliceSpace.SpaceID)
	require.NoError(t, err)
	require.False(t, messageLikesUnread)
}
...
func TestSpaceReadMarkersDriveSplitUnreadState(t *testing.T) {
	ctx := context.Background()
	module := newSpaceTestModule(t)
	...
	require.NoError(t, module.Read.UpsertMessageThreadReadMarker(ctx, aliceID, aliceSpace.SpaceID, bobSpace.SpaceID, 2000))
	conversations, _, err = module.Messages.ListConversations(ctx, aliceID, aliceSpace.SpaceID, "", 10)
	require.NoError(t, err)
	require.False(t, conversations[0].Unread)
	require.Equal(t, int64(0), conversations[0].UnreadCount)
	...
	require.NoError(t, module.Read.UpsertNotificationsReadMarker(ctx, aliceID, aliceSpace.SpaceID, 500))
	notificationsUnread, err = module.Messages.HasUnreadNotifications(ctx, aliceSpace.SpaceID)
	require.NoError(t, err)
...
func TestSpaceReadMarkersUseThreadAndGlobalScopes(t *testing.T) {
	ctx := context.Background()
	module := newSpaceTestModule(t)
	...
	bobLatestActivityAt, err := module.Messages.GetLatestMessageThreadActivityAt(ctx, aliceID, aliceSpace.SpaceID, bobSpace.SpaceID)
	require.NoError(t, err)
	require.Equal(t, int64(1001), bobLatestActivityAt)
	require.NoError(t, module.Read.UpsertMessageThreadReadMarker(ctx, aliceID, aliceSpace.SpaceID, bobSpace.SpaceID, bobLatestActivityAt))
	bobConversation = conversationBySpaceID(bobSpace.SpaceID)
	require.True(t, bobConversation.Unread)
	...
	require.NoError(t, module.Read.UpsertMessageThreadReadMarker(ctx, aliceID, aliceSpace.SpaceID, charlieSpace.SpaceID, 2000))
	messagesUnread, err = module.Messages.HasUnreadMessages(ctx, aliceSpace.SpaceID)
	require.NoError(t, err)
	require.False(t, messagesUnread)
	notificationsUnread, err = module.Messages.HasUnreadNotifications(ctx, aliceSpace.SpaceID)
	require.NoError(t, err)
	require.True(t, notificationsUnread)
	require.NoError(t, module.Read.UpsertNotificationsReadMarker(ctx, aliceID, aliceSpace.SpaceID, 200))
	notificationsUnread, err = module.Messages.HasUnreadNotifications(ctx, aliceSpace.SpaceID)
	require.NoError(t, err)
	require.False(t, notificationsUnread)
	require.NoError(t, module.Read.UpsertMessageLikesReadMarker(ctx, aliceID, aliceSpace.SpaceID, 1002))
	bobConversation = conversationBySpaceID(bobSpace.SpaceID)
...
func TestUnreadScopesTrackReadableActivityWithoutChangingLatestPreview(t *testing.T) {
	ctx := context.Background()
	module := newSpaceTestModule(t)
	...
	require.NoError(t, err)
	require.Equal(t, "post_like", notifications[0].Type)
	require.Equal(t, alicePostID, notifications[0].Post.PostID)
	require.Equal(t, int64(6000), notifications[0].CreatedAt)
	require.True(t, notifications[0].Unread)
	notificationsUnread, err = module.Messages.HasUnreadNotifications(ctx, aliceSpace.SpaceID)
	require.NoError(t, err)
	require.True(t, notificationsUnread)
	...
	require.NoError(t, err)
	require.True(t, notificationsUnread)
	require.NoError(t, module.Read.UpsertNotificationsReadMarker(ctx, aliceID, aliceSpace.SpaceID, 6000))
	notificationsUnread, err = module.Messages.HasUnreadNotifications(ctx, aliceSpace.SpaceID)
	require.NoError(t, err)
	require.False(t, notificationsUnread)
	notifications, _, err = module.Messages.ListNotifications(ctx, aliceSpace.SpaceID, "", 10)
	require.NoError(t, err)
	require.False(t, notifications[0].Unread)
}
