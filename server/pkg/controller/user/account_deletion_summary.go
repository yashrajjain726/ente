...
func (c *UserController) GetAccountDeletionSummary(ctx context.Context, userID int64) (details.AccountDeletionSummaryResponse, error) {
	...
	g.Go(func() error {
		count, err := c.UserCacheController.GetUserFileCountWithCache(userID, ente.Photos)
		if err != nil {
	...
	g.Go(func() error {
		count, err := c.UserCacheController.GetUserFileCountWithCache(userID, ente.Locker)
		if err != nil {
